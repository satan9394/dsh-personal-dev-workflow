...
- One tension is left for the repo owner: the test script cleans its own temp directories with `fs.rmSync` (Node has no zero-dependency recycle-bin API). Scope is limited to directories it just created under `os.tmpdir()` with a double prefix guard, but it does sit against the "deletions go to the recycle bin" rule.
