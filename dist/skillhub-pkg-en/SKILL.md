...
2. **Two-level budget** (defaults; override per run/Mission and record it in `RUN_STATE.md`). **Run Budget** (`## Budget`; 8 counter lines whose labels map verbatim to the `init` scaffold and the `advance` aliases) guards context pollution, token blow-up and over-long sessions: `Epoch` ≤ 2 · `完成卡数` ≤ 6 · `已用 Repair` ≤ 1 · `已派子代理` ≤ 8 · `WorkSet 规模` ≤ 8 · `Worker 数` ≤ 3 · `Research pass` ≤ 1 · `子代理嵌套` ≤ 1 — exhausted → write `RUN_STATE.md`, run `new-run` (fresh context) and continue, **no human confirmation**.
