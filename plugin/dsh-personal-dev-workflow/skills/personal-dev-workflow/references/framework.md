...
（来源：社区自研 harness-engineering skill；LangChain 数据：只改 harness，Terminal Bench 52.8% → 66.5%。）
...
- **成本数字**（社区自研 loop-engineering）：无记忆循环每轮 30k-50k token，有记忆 5k-15k；通过率 <50% 说明只是把评审垃圾变多了。
- **熔断**：同一问题修复 3 次仍不过 → 清空上下文重新派活或换执行器（防 Ralph Wiggum 空转）。
- **编排决策三问**（社区自研 agent-teams）：谁来协调 / 子任务独立吗 / 会改同一批文件吗；能单不多。
